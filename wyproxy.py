    ...
    def response(self, f):
        ...
        if not self.unsave_data:
            try:
                ...
                #     new_target_dict.add(parser.parser_data())
                # for data in new_target_dict:
                #     checker = SqlCheck(target_dict=data)
                #     checker.put_url_into_queue()
                #     checker.run()
                #     mysqldb_io.insert_result(data)
                if parser.parser_data():
                    checker = SqlCheck(target_dict=parser.parser_data())
                    checker.put_url_into_queue()
                    checker.run()
